...
class WordDirRelsDirDocumentStructureDocFile extends StructureDocFile {
    ...
    const TYPE_HYPERLINK = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink';
     ...
     */
    public function registerHyperLink($target, $isExternal = true) {
        $targetMode = $isExternal ? 'External' : null;
        return $this->registerRels($target, self::TYPE_HYPERLINK, $targetMode);
    }
     ...
     * @param string $target
     * @param string $type
     * @param string $targetMode External
     * @return array
     */
    public function registerRels($target, $type, $targetMode = null) {
        if ($this->validateType($type)) {
            $key = 'rId' . (sizeof($this->stack) + 1);
            $this->stack[$key] = array($target, $type, $key, $targetMode);
    ...
    public function getContent() {
        ...
        foreach ($this->stack as $item) {
            list($target, $type, $key, $targetMode) = $item;

            $strings[] = '<Relationship Id="' . $key . '" Type="' . $type . '" Target="' . $target . '" ';
            if (!is_null($targetMode)) {
                $strings[] .= 'TargetMode="External"';
            }
            $strings[] .= '/>';
        }
